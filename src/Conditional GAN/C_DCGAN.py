...
# Result directory
result_root = './src/Conditional GAN/results/C_DCGAN/'
...
class C_DCGAN:
    def __init__(self, device, ngpu, classes, nz=100):
    ...
    def train(self, dataloader,
        ...
        for epoch in range(num_epochs):
            ...
            for i, (data, labels) in enumerate(dataloader, 0):
                ...
                # TODO: change the training loop based on the new label added to the dataset.
                #  The dataloader format has changed due to the addition of the labels to the dataset
                ...
                for i in range(d_iters):
                    # Forward pass
                    output = self.netD(images, d_labels).view(
                        -1)  # The real images are sent to the discriminator and the output is calculated
                    errD_real = self.criterion(output, y_real)  # The loss for the real images is calculated
                    real_accuracy = torch.mean(1 - torch.abs(errD_real - y_real)).item()
                    ...
                    fake = self.netG(noise, g_labels)  # The fake images are generated

                    # Classify the fake images with the discriminator

                    # Forward pass
                    output = self.netD(fake.detach(), d_labels).view(-1)
                    errD_fake = self.criterion(output, y_fake)  # The loss for the fake images is calculated
                    fake_accuracy = torch.mean(1 - torch.abs(errD_fake - y_fake)).item()
                    # Backward pass
                ...
                for i in range(g_iters):
                    ...
                    errG = self.criterion(output, y_real)  # The loss for the generator is calculated
                    # Backward pass
                ...
                batch.set_postfix({'Disc loss': torch.mean(torch.tensor(disc_losses[-d_iters:])).item(),
                                   'Gen loss': torch.mean(torch.tensor(gen_losses[-d_iters:])).item(),
                                   'Disc Real Accuracy': torch.mean(
                                       torch.tensor([disc_accuracies[-i - 1][0] for i in range(d_iters)])).item(),
                                   'Disc Fake Accuracy': torch.mean(
                                       torch.tensor([disc_accuracies[-i - 1][1] for i in range(d_iters)])).item()})
...
if __name__ == '__main__':
    ...
    model = C_DCGAN(device=device, ngpu=ngpu, classes=classes)
    run_name = 'run_2023-09-17_19-59-27'
